...
### 2. Split module into `waves.lua` and `feedback.lua`
...
### 3. Document `retaliation_state_key` format coupling
...
### 4. Annotate `connected_force_players` as intentionally private
...
### 5. Remove `module.storage()` and `module.storage_feedback()` once debug layer exists
